...
class Contests(commands.Cog):
    ...
    def _getSeeds(self, registrants, handles):
        MAX = 6144

        # Precompute the ELO win probability for all possible rating differences.
        elo_win_prob = np.roll(1 / (1 + pow(10, np.arange(-MAX, MAX) / 400)), -MAX)

        # Compute the rating histogram.
        count = np.zeros(2 * MAX)

        allhandles = []
        for a in registrants:
	        try:
	            allhandles.append([a["handle"], int(a["rating"])])
	            count[int(a["rating"])] += 1
	        except:
	        	continue

        # Precompute the seed for all possible ratings using FFT.
        seeds = 1 + ifft(fft(count) * fft(elo_win_prob)).real

        ToPlot = []
        for a in allhandles:
            if a[0] in handles:
                ToPlot.append([a[0], a[1], int(seeds[a[1]])])

        return ToPlot

    def _make_standings_seed(self, ToPlot):
        ToPlot = sorted(ToPlot, key=lambda x: [x[2], -x[1]])
        content = "```cs\n#      Handle             Rating    Seed\n------ ------------------ --------- ------\n"
        cnt = 0
        for i in ToPlot:
            cnt += 1
    ...
    @commands.command(brief='Show seed based ranklist for given handles and/or server members.')
    async def seedranklist(self, ctx, contest_id: int, *handles: str):
        """Shows seed based ranklist for the contest with given contest id. If handles contains
        '+server', all server members are included. No handles defaults to '+server'.
        """
        ...
        ToPlot = self._getSeeds(registrants, handles)
        if ToPlot == []:
            if wait_msg:
                try:
                    await wait_msg.delete()
                    ...
                    pass
            raise ContestCogError(f"0 handles were given")

        content = self._make_standings_seed(ToPlot)
